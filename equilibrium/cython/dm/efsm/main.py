...
def solve(w, reputations, granularity=10000):
    ...
    while high - low > epsilon:
        ...
        k, c = estimate_kc(guess, lowers)

        try:
            costs = efsm_internal.solve(k, c, lowers, uppers, bids).T
        except Exception:
...
def estimate_kc(b_lower, lowers):
    n = lowers.size
    sums = np.zeros(n, np.float)

    for k in np.arange(n):
        for i in np.arange(k):
            sums[i] = 1 / (b_lower - lowers[i])

        c = b_lower - (k - 1) / np.sum(sums)
